...
class ConditionalDataset(torch.utils.data.Dataset):
  def __init__(self, paths):
    ...
    for path in paths:
      ...
      # 这是获取所有的wav文件，嗯？梅尔谱文件被直接无视了吗... NOTE TODO
      # 并没有，后续还是会继续去找已经通过pre-process得到的梅尔谱的!
  ...
  def __getitem__(self, idx):
    ...
    # NOTE 哦，明白了，这是根据wav文件名，去找的 wav.spec.npy
    #这个保存了梅尔谱的文件名了, shape = torch.Size([1, 42736])
...
class Collator:
  ...
  def collate(self, minibatch): 
    # minibatch = a list of dicts, each element is a dict, 
    #{'audio': tensor, 'spectrogram': array}, collate = 检查校对 的意思
    #import ipdb; ipdb.set_trace()
    ...
    for record in minibatch:
      if self.params.unconditional:
          ...
          # 截取，终点, e.g., 138, (开区间，只到137即可)

          record['spectrogram'] = record['spectrogram'][start:end].T 
          # NOTE, 如果长度超过了62，那么只需要保留62的长度就可以了.

          start *= samples_per_frame 
          # 256，大意是，一个梅尔谱frame里面，对应了256个采样点, so, 76*256=19456

          end *= samples_per_frame # =138*256=35328

          record['audio'] = record['audio'][start:end] 
          # torch.Size([15872]), 大概是1秒, 
          # 这是截取了一部分的原始wave form里面的数据

          record['audio'] = np.pad(record['audio'], 
                  (0, (end-start) - len(record['audio'])), 
                  mode='constant') # 这里什么都没有做
  ...
  def collate_gtzan(self, minibatch):
    ...
    for data in minibatch:
      if data[0].shape[-1] < mean_audio_len:  # pad
        data_audio = F.pad(data[0], 
                (0, mean_audio_len - data[0].shape[-1]), mode='constant', value=0)
...
def from_path(data_dirs, params, is_distributed=False):
  ...
  return torch.utils.data.DataLoader(
      ...
      num_workers=0, #os.cpu_count()=80 啊，有钱! 居然有80个内核... NOTE
