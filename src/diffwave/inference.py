...
def predict(spectrogram=None, model_dir=None, 
  ...
  with torch.no_grad():
    ...
    if not model.params.unconditional:
      ...
      audio = torch.randn(spectrogram.shape[0], 
              model.params.hop_samples * spectrogram.shape[-1], device=device)
    else:
...
def main(args):
  ...
  audio, sr = predict(spectrogram, 
    model_dir=args.model_dir, fast_sampling=args.fast, params=base_params)
